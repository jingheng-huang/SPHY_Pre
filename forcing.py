...
class processForcing():
    ...
    def createPrecDB(self):
        ...
        # I change this code - hjh
        if self.dbSource == 'default':
        #if self.dbSource == :
            ...
            for i in range(0, self.timeSteps):
                ...
                print self.precDBPath + 'Prcp.nc'
                com = 'gdal_translate -ot Float32 -of GTiff -b ' + str(daynr) + ' ' + self.precDBPath + 'Prcp.nc' + ' ' +  self.tempdir + 'temp.tif'
                ...
                com = self.pcrasterModelFile('"' + self.outdir + 'prec' + pcrstr + '" = if("' + self.tempdir + 'clone.map", 1 * 1 * "' + self.tempdir + 'temp.map")')
    ...
    def createTempDB(self):
        ...
        if self.dbSource == 'default':
            ...
            # com = 'gdal_translate  -ot Float32 -of GTiff ' + self.dbDem + ' ' + self.tempdir + 'temp.tif'
            com = 'gdalmanage copy  ' +  self.dbDem + ' ' + self.tempdir + 'temp.tif' # hjh
            #com =  'gdal_translate NETCDF:'+'"'+ self.dbDem + '"'+':elevation '+ self.tempdir + 'temp9999.tif' # make this more clear - john
            commands.append(com)
            ...
            forcings = {'Tair': self.tavgDBPath, 'Tmin': self.tminDBPath, 'Tmax': self.tmaxDBPath}
            for f in forcings:
                for i in range(0, self.timeSteps):
                    daynr = i+1 # required for pcraster extension
                    curdate = self.startDate + datetime.timedelta(days=i)
                    ...
                    #     year + month + '.nc ' + self.tempdir + 'temp.tif'
                    com = 'gdal_translate -ot Float32 -of GTiff -b ' + str(daynr) + ' ' + forcings[f] + f + '.nc ' + self.tempdir + 'temp.tif'
                    commands.append(com)
                    #-command to project, resample, and extract to the correct extent
                    com = 'gdalwarp -s_srs ' + self.dbSrs + ' -t_srs ' + self.t_srs + ' -te ' + \
                        self.xMin + ' ' + self.yMin + ' ' + self.xMax + ' ' + self.yMax + ' -tr ' + \
                    ...
                    com = self.pcrasterModelFile('"' + self.outdir + f + pcrstr + '" = ("' + self.tempdir + \
                        'temp.map" + (-0.0065 * "' + self.tempdir + 'demDiff.map")) - 0')
                    commands.append('pcrcalc -f ' + com)
                    self.subProcessing(commands)
                    #-Progress bar
                    self.counter += 1
